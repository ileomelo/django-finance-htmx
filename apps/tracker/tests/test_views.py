...
def test_values_appear_on_list_page(user_transactions, client):
  user = user_transactions[0].user

  client.force_login(user)

  income_total = sum(t.amount for t in user_transactions if t.types == "income")
  expense_total = sum(t.amount for t in user_transactions if t.types == "expense")
...
def test_transaction_type_filter(user_transactions, client):
  ...
  for transaction in qs:
    assert transaction.types == "income"
  ...
  for transaction in qs:
    assert transaction.types == "expense"
